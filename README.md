...
- 提供 AstrBot Plugin Pages 运维面板，可查看概览、分组订阅、暂存队列、镜像测试和缓存清理。
...
- `分组订阅`：查看各用户分组的关注账号、推送目标、无效推送目标、定时策略和分组提示；支持导入和删除关注账号。
...
WebUI v1 不替代 AstrBot 设置页；Nitter 实例、媒体限制、AI provider、提示词、并发与限流等复杂配置仍在 `_conf_schema.json` 对应的 AstrBot 配置界面维护。
