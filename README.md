...
`probe_proxy_fetch.py` 只要求提供一个代理 URL，默认通过该代理抓取 `nasa` 1 条带作者媒体的推文，并让 Nitter RSS、xdown 解析、视频探测和媒体下载全程共用该代理。也可追加用户名和数量，例如 `python scripts\probe_proxy_fetch.py http://127.0.0.1:8080 NASA 2`。脚本日志不会主动输出代理凭据；但认证 URL 仍会留在 PowerShell 历史和本机进程参数中，测试后应清理对应历史记录，避免在共享机器上直接使用长期凭据。
...
| `proxies` | 基础设置中的可选代理列表；可填写显示名称区分不同代理，支持 HTTP、HTTPS、SOCKS5、SOCKS5h 和用户名密码，启用项按添加顺序故障切换。 |
...
| `media_retry_attempts` | xdown 解析和媒体下载遇到临时错误时的总尝试次数，默认 `3`。 |
| `media_retry_delay_seconds` | 媒体网络重试之间的固定等待秒数，默认 `5`。 |
| `translate_enabled` | 是否翻译非中文推文。 |
| `comment_enabled` | 是否按概率追加 AI 评论。 |
| `vision_enabled` | 是否启用 AI 识图；结果主要作为 AI 评论上下文。 |

`proxies` 作用于 Nitter RSS、xdown、视频探测和媒体下载，不代理 QQ、Telegram、Lark 等平台发送。全部启用代理失败时不会回退直连；列表为空或全部关闭时保持原有网络行为（包括运行环境代理）。AstrBot 标准设置页按添加顺序保存代理且暂不支持拖拽重排；代理密码会以普通字符串保存和展示，请勿分享相关配置文件或截图。插件日志不会输出代理用户名、密码或媒体下载 URL 的查询参数。
