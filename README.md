...
- `分组订阅`：改为左侧分组列表 + 右侧详情编辑；支持新建分组、编辑分组名称/启停/每日检查/暂存开关/纯文本过滤，并继续支持导入和删除关注账号。
...
`group_id` 在 WebUI 中只读展示，默认分组不可删除。`检查间隔分钟数` 和 `暂存发布时间` 仍是全局配置，页面里只读显示为“继承全局”；`推送目标` 在这一阶段仍为只读展示。
