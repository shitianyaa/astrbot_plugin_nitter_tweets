...
| 后台推送 | 按 `tweet_groups` 分组订阅、定时检查并即时推送。 |
| 平台发送 | QQ/OneBot 支持合并转发；Lark、Telegram、微信 OC 和其他平台走普通发送。 |
| 媒体与 AI | 支持图片附件；可选开启视频/GIF 和翻译。 |
...
- `分组订阅`：维护分组名称、启停、每日检查、纯文本过滤、关注账号和推送目标。
...
- 手动 `/推文 用户名 数量` 不写入推送记录或扫描水位；后台检查会比较当前 RSS 首屏约 20 条与该账号的 seen 记录，首屏未出现上次基准时按 `Min-Id` 翻页直到找到基准，并发送所有差集。
