...
- 定时检查会先确认本轮需要推送的账号数，再按账号顺序逐条处理和发送；QQ/`aiocqhttp` 合并转发目标仍按阈值缓冲到本轮最后统一发送。
...
- 手动 `/推文` 会按单条推文处理：一条推文完成翻译、媒体下载、AI 识图和 AI 评论后就发送这一条。定时推送会先完成本轮需要推送账号的发现，用于标题显示“所有账号 x/总数”；随后普通目标仍按账号顺序逐条处理和发送，并在同一账号内显示“该账号推文 a/总数”。手动 `/推文 用户名 数量` 的普通单条结果标题会显示“本次结果 x/总数”；识图结果仅用于生成评论上下文，不单独展示；QQ 合并转发目标仍会缓冲到本轮最后统一发送。
