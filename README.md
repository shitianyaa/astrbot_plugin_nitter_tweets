...
完整默认值见 [_conf_schema.json](./_conf_schema.json)。AstrBot 设置界面已按“基础、媒体、AI 翻译、AI 评论、AI 识图、后台检查、暂存发布、推送目标与用户分组、并发与限流”分组展示；旧版本的扁平配置仍会兼容读取，并自动迁移到默认分组。`并发与限流` 目前是预留栏目，多推文抓取、翻译、评论和发送仍按当前串行流程与发送间隔执行；单条推文内多张图片识图会并发调用模型。
...
“最近最多 30 条”表示本次最多拉取 30 条；“本次结果 1/30”表示当前正在发送手动查询结果中的第 1 条。QQ 合并转发达到阈值时仍会合并发送。
