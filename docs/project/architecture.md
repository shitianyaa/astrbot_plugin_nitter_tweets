...
5. Blogger 按 RSS 首屏与基准分页；Tag 按 `search_instances` 串行取固定候选；List 按旧水位继续翻页，已初始化且第一页有有效状态 ID 时达到页数上限仍未结束才自动重建基线并跳过本轮发送，否则保留旧基线或按分页失败处理。Tag 使用 `q:`、List 使用 `list:` seen 键。
...
10. 分页完整扫描后才发送新推文；已初始化 List 分页预算耗尽且第一页有有效状态 ID 时自动重建第一页基线并跳过本轮，准备或发送失败时不跨过未送达推文，并清理普通缓存。
