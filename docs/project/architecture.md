...
5. Blogger 按 RSS 首屏与基准分页；Tag/List 都把旧扫描水位传给 HTML 分页器，在 `html_max_pages` 范围内寻找基准。扫描未完整且当前第一页有有效状态 ID 时，按 `max_tweets_per_check` 处理：上限为 0 时不推送并自动重建第一页基准，上限大于 0 时最多推送该数量，所有目标成功后自动重建第一页基准；发送、基准无效或写入失败时保留旧水位。Tag 使用 `q:`、List 使用 `list:` seen 键。
