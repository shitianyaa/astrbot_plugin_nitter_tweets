...
这是全局 HTML 配置，控制一次 List 抓取最多请求多少页。每翻一页通常会增加一次页面 GET，不包含门禁请求。首次订阅只取最多 20 条建立基线；已有水位后本轮扫描可以超过 20 条，并继续翻页到命中旧水位或游标结束，但持久化水位仍最多保存最新 20 个 ID。达到 `html_max_pages` 后仍有后续游标时，已初始化且第一页有有效状态 ID 的 List 会自动用第一页最多 20 个状态 ID 重建扫描基线，本轮不发送，并提示旧积压可能被跳过；如果第一页没有有效状态 ID，则保留旧基线；未初始化 List 仍按分页失败处理。默认 `1`，公共实例可按更新量和限流情况设置为 `1-3`。
...
首次订阅取最多 20 条建立基线；已有水位则翻页到旧水位或游标结束
  ↓
已初始化且第一页有有效 ID 的 List 达到 html_max_pages 仍未完整扫描时自动重建基线，本轮不发送；无有效 ID 时保留旧基线，未初始化 List 按分页失败处理
  ↓
按全局与分组双层开关过滤转发
  ↓
过滤纯文本（如果分组启用）
...
5. 检查是否出现“自动重建基线提示”或“自动重建基线未完成”；如频繁重建，可适当提高 `html_max_pages`
6. 使用 `/推文状态` 查看分组检查状态
