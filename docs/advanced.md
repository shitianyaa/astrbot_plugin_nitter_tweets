...
- 未通过 UMO 校验的推送目标 / 博主订阅源（含历史遗留的无效条目）会在分组详情中以虚线 chip 展示并标记「无效」；它们不会收到推送，但原文随保存保留在配置里，可随时显式移除或修正后重填。添加推送目标直接生效（不再弹确认框）；移除与删除类操作仍需确认。
...
- 手动 `/推文` 与 `/推图` 遵循全局 `filter_reposts_enabled` 开关；`/镜像测试` 始终保留转发，便于完整查看镜像返回内容；`/推文搜索` 保持过滤纯转推，不读取分组开关。
...
- 转推：手动 `/推文` 与 `/推图` 遵循全局 `filter_reposts_enabled` 开关；`/镜像测试` 始终保留转发；`/推文搜索` 固定去掉纯转推。Blogger、Tag、List 分组定时检查仅在全局与分组的 `filter_reposts_enabled` 都开启时过滤。
