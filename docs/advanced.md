...
- `最近推送` 中的失效分组检测只列出存在于推送历史、但当前 `tweet_groups` 不存在的 `group_id`；删除前需要确认，删除范围包括该 `group_id` 的推送历史、防重复推送记录和暂存运行数据。
...
| `proxies` | 可选代理列表（`template_list`），支持多条顺序故障切换。 |
...
- 代理覆盖 Nitter RSS、xdown 解析、视频探测和媒体下载，不代理 QQ、Telegram、Lark 等平台发送。所有启用代理均失败时直接报错，不回退直连；列表为空或全部关闭时保持原有 `urllib` 网络行为，包括环境变量代理。
- AstrBot 标准设置页当前不支持拖拽调整 `template_list` 顺序；需要改变优先级时按目标顺序删除后重新添加，或直接编辑插件配置文件。
- AstrBot 当前把插件 Schema 的 `string` 字段作为普通文本保存和展示，代理密码不会自动掩码。请勿分享配置文件或包含密码字段的截图；插件日志不输出代理用户名、密码或媒体下载 URL 的查询参数。
- HTTP 和 SOCKS5/SOCKS5h 的用户名密码在客户端到代理的链路上不自带加密，只应连接可信代理；`https` 代理会加密这一段链路。
- 代理请求由 `requests>=2.33,<3` 处理，SOCKS 支持依赖 `PySocks>=1.7.1,<2`；HTTPS 代理证书和目标站证书保持默认验证，不会关闭 TLS 校验。
...
| `media_retry_attempts` | xdown 解析和媒体下载遇到临时网络错误时的总尝试次数，默认 `3`，范围 `1-10`。每次文件下载重试都会从头开始。 |
| `media_retry_delay_seconds` | xdown 解析和媒体下载重试之间的固定等待秒数，默认 `5`，范围 `0-60`；代理列表内部切换不额外等待。 |
...
- 旧版顶层 `watch_users`、`push_targets` 和分组相关定时配置会自动迁移到 `default` 默认分组；`tweet_groups` 中的各用户分组会独立运行，并拥有独立的推送记录。
...
`scripts/probe_proxy_fetch.py` 用于验证代理完整链路。代理 URL 是唯一必填参数，默认抓取 `nasa` 1 条带作者媒体的推文，并让 Nitter RSS、xdown、视频探测和媒体下载共用同一个 `NetworkClient`。默认使用临时缓存并在结束时清理；使用 `--keep-dir 目录` 可复制通过文件头校验的媒体。示例：

```text
python scripts\probe_proxy_fetch.py http://127.0.0.1:8080 NASA 2
python scripts\probe_proxy_fetch.py socks5h://user:password@127.0.0.1:1080 --keep-dir tests\downloads
```

脚本日志不会主动回显代理凭据。凭据包含 `@`、`:`、`/` 等字符时需要先进行 URL 编码；认证 URL 仍会进入 PowerShell 历史和本机进程参数，测试后应清理对应历史记录，不要在共享机器上直接使用长期凭据。
