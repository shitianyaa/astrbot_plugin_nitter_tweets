...
| `media_only_enabled` | 是否只发送作者和成功准备的图片/视频/GIF；受全局媒体类型开关和 `max_media_per_tweet` 控制。全局媒体不可用时只在 WebUI 和日志提示，并自动回退完整内容；手动命令和历史重推不受影响。 |
...
- 分组“仅媒体”有效时，消息只包含 `@作者` 和已准备附件；正文、翻译、原帖链接、媒体 warning 和 AI 提示都不会进入消息。没有可发送附件的推文整条跳过；临时解析/下载失败下轮重试，明确策略排除允许扫描基准继续推进。
