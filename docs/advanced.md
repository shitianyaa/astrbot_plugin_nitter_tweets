...
- 目标平台拒收内容本身时（NapCat 群媒体上传后收不到回执的 `Timeout: NTEvent ... sendMsg`、合并转发 `res_id` 拒绝），插件标记为不可重试：传输编码梯度与组件级重试都立即停手，不再用相同字节空跑。这类拒收由内容触发，换编码、换下载渠道或降低 `media_quality` 都不改变判定。
