...
def test_list_pagination_respects_configured_page_budget(monkeypatch):
    pool = _pool(filter_reposts=False, max_pages=1)
    pool._get_html = MagicMock(return_value=b"page-1")
    monkeypatch.setattr(
        pool_module,
        "parse_timeline_html",
        lambda _body, _base, source="": SimpleNamespace(
            tweets=[_tweet("10")],
            next_cursor="cursor-2",
            raw_item_count=1,
        ),
    ...
    result = pool._paginate_list("https://a.example", "12345", 20)

    assert [tweet.status_id for tweet in result] == ["10"]
    assert result.scan_complete is True
    assert pool._get_html.call_count == 1


def test_list_pagination_uses_page_budget_for_incomplete_watermark_scan(monkeypatch):
    pool = _pool(filter_reposts=False, max_pages=1)
    pool._get_html = MagicMock(return_value=b"page-1")
    monkeypatch.setattr(
        pool_module,
    ...
    assert result.anchor_status_ids == ["10", "9"]
    assert pool._get_html.call_count == 1
