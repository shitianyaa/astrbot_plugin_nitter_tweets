...
- 修复已初始化 List 订阅的本轮扫描在前 20 条处截断并提前推进水位的问题：现在会继续翻页到命中旧水位或游标结束；达到 `html_max_pages` 仍未完成时，自动用第一页最多 20 个状态 ID 重建基线，本轮不发送并提示旧积压可能被跳过。若第一页没有有效状态 ID，则保留旧基线。首次基线和后续持久化水位仍最多保存 20 个 ID。
