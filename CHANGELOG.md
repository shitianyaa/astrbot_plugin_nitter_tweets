...
- **Dashboard 分组订阅：无效条目可见可删，保存不再静默清除**：
  - 未通过 UMO 校验的推送目标与博主订阅源（含历史遗留的无效条目）现在会在分组详情中以虚线 chip 呈现并标记「无效」，可随时显式移除或修正后重填；此前这些条目进"隔离区"后无任何查看/删除出口，右上角「N 异常」计数只增不减。
  - 保存分组任意字段、批量导入/移除、聊天订阅命令都不再静默清除配置中的无效条目；删除的唯一途径是编辑器中的显式移除。
