...
- 新增 List 分组支持：可通过配置文件或 WebUI 订阅 Public List，复用 `search_instances` HTML 抓取并独立保存 seen/扫描水位。

### Fixed

- 修复 List 更新较多时分页预算耗尽后每轮重复从第一页扫描、积压持续增长的问题：已有水位的 List 会在未命中旧基线时自动用第一页状态 ID 重建基线，本轮跳过发送，并在日志/检查摘要中明确提示旧积压可能被跳过；网络或解析硬失败不会误触发重建。

### Changed

- 明确 `html_max_pages` 为每轮 HTML/List 扫描预算（运行时限制 `1-5`），并将 List 分页结果与第一页基线 ID 传递到调度器；本次不新增数据库结构或迁移。
