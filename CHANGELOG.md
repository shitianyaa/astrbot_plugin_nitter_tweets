...
- **Dashboard 顶部反馈交互全面重做**：
  - 顶部结果/进行中提示固定于顶栏下方并支持手动关闭；关闭进行中提示仅隐藏 UI，不打断后台操作，下一次操作仍正常显示。
  - 顶部提示重构为单一通知栈：成功/警告/失败与进行中提示垂直排列、互不重叠，每条自带关闭按钮；结果提示 4 秒自动消失，重复提示仅刷新停留时长，最多同时保留 4 条。
  - 动作结束时结果提示原地改写进行中节点，消除「移除后新增」的位置跳动；进行中提示不再挂自动消失计时，长动作的顶部指示不会中途消失。
  - 新增 `--sticky-top` 统一吸附线：推送历史工具栏与插件配置操作栏吸顶时按通知栈实测高度自动让位，修复配置页滚动后通知原位盖住保存/重置栏的问题；配置分组导航同步让位。
  - 删除废弃分组残留历史后自动重新检测并刷新孤儿列表（原先因动作互斥静默跳过），刷新失败给出明确警告。
  - 错误提示使用 `role="alert"` 语义播报，读屏兼容性更好，并清理旧横幅方案遗留的无用样式。
- **Dashboard 分组订阅：无效条目可见可删，保存不再静默清除**：
  - 未通过 UMO 校验的推送目标与博主订阅源（含历史遗留的无效条目）现在会在分组详情中以虚线 chip 呈现（订阅源直接标记「无效」，推送目标经「测试连通性」探测后标记），可随时显式移除或修正后重填；此前这些条目进"隔离区"后无任何查看/删除出口，右上角「N 异常」计数只增不减。
  - 保存分组任意字段、批量导入/移除、聊天订阅命令都不再静默清除配置中的无效推送目标与博主订阅源（tag/list 分组的无效 watch_queries / watch_lists 维持现状）；删除的唯一途径是编辑器中的显式移除。
  - 添加推送目标/订阅源的查重覆盖有效 + 无效条目，杜绝同一无效条目反复添加累积。
  - 「测试连通性」对纯无效目标的分组可用，并在按钮旁显示连通性汇总徽章；探测结果在保存/删除分组/手动刷新后自动失效。

### Changed & Improved

- **Dashboard 分组编辑器性能**：
  - 击键不再每字符全量重建分组列表（rAF 合帧 + 脏检查缓存），大配置下打字卡顿消除。
  - 保存流水线的网络请求从 3 个减为 2 个（保存响应直接回填最新分组，失败自动降级全量刷新）。
  - 实例探测与黑名单操作只刷新目标区块，不再整棵重建编辑器。
  - 添加推送目标不再弹确认框（纯增量可逆操作，输错可显式移除）；移除与删除类操作仍保留确认。
